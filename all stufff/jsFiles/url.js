...
window.addEventListener('click', (event)=>{
    ...
    window.location.hash = '#basketPage';
    ...
    case 'menuA':{window.location.hash ='#catalog';break;}
    case 'logo':{window.location.hash='#';break;}
    default: {break;}
    }
});
...
function validation(textArea){
    ...
    if (!phoneNumber.test(textArea[0].value)){window.alert('Некоректний номер телефону');isValid=false;textArea[0].style.color='#e29467';}
...
function refreshCounter(){
    ...
    if (orderTmp == null){
        order = []; localStr.setItem('orders',  JSON.stringify( order));}
    else{ 
...
window.addEventListener('hashchange', async ()=>{location.reload();});

//export * as url from 'url.js';
export { retouchText, isDate, validation,pushOrder, addToOrder, deleteFromOrder,  refreshCounter, findPrice,screanRender, Order,
    onReady};